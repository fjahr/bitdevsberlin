---
layout: post
title: "Socratic Seminar #82"
---

[Meetup](https://www.meetup.com/de-de/bitcoin-lab-berlin/events/315196279/)

Thanks to [Ordimint](https://ordimint.com) for sponsoring food and drinks for this meetup!

Note: There will be a [partial solar eclipse](https://de.wikipedia.org/wiki/Sonnenfinsternis_vom_12._August_2026) during the meeting and we may take a break and step outside for a while.

## Special: BIP110

- [BIP110 Situation Monitor](https://bip110.orange.surf/live.html)
- [BIP110 Signaling](https://wickedsmartbitcoin.com/bip110_signaling)
- [Current user agents on the network](https://bitnod.es/)
- [Choosing the Core chain? Here is how to switch](https://bip110.org/change-chains/)
- [Does a soft fork always succeed?](https://bitcoin.stackexchange.com/questions/130775/does-a-softfork-always-succeed)
- [Testnet4 picks new PoW Algorithm](https://x.com/coinjoined/status/2087145713947811891)

## Special: Coldcard Hack and Fallout

- [Full panic - one of my wallets was drained](https://www.reddit.com/r/Bitcoin/comments/1vatgl4/full_panic_one_of_my_wallets_was_drained/)
- [Kevin Loaec post](https://x.com/KLoaec/status/2082926304995762209)
- [Rob Hamilton post](https://x.com/Rob1Ham/status/2082896614218203616)
- [Predictable RNG Fallback and 32-Bit Reseed in COLDCARD Firmware](https://engineering.block.xyz/blog/predictable-rng-fallback-and-32-bit-reseed-in-coldcard-firmware)
- [Technical Deep Dive into the Entropy Issue](https://blog.coinkite.com/entropy-technical-backgrounder/)
- [Wizardsardine Report on ColdCard](https://wizardsardine.com/blog/coldcard-rng-vulnerability/)
- [Wizardsardine Deep Dive](https://wizardsardine.com/blog/coldcard-vuln-deep-dive/)
- [Entropy visualizer](https://hashexplained.com/entropy)
- [Coldcard Watch](https://coldcardwatch.com/)
- [CK Tripwire](https://cktripwire.com/)
- [Security Advisory: Update BTCPay Server to 2.4.2 Immediately](https://blog.btcpayserver.org/[118;1:3usecurity-advisory-btcpay-server-2-4-2/)
- [Red Teaming Bitcoin](https://x.com/callebtc/status/2085024458012586286)

## Security/CVEs/InfoSec/Research

- [Vulnerability Disclosure: Twin Memory Exhaustion DoS Vulnerabilities in Core Lightning](https://delvingbitcoin.org/t/vulnerability-disclosure-twin-memory-exhaustion-dos-vulnerabilities-in-core-lightning/2731)
- [MPFlow: Learning Budgeted Max-Flow Optimization on the Lightning Network with Deep Graph Reinforcement Learning](https://arxiv.org/abs/2607.08703)
- [How Reliable Is the Multi-Input Heuristic for Bitcoin Address Clustering in Law Enforcement Contexts?](https://arxiv.org/abs/2607.07414)
- [Decentralization and Governance in IoT: Bitcoin and Wikipedia Case](https://arxiv.org/abs/2607.06784)
- [Open Bitcoin Metrics: Verifiable Full-Node-Derived Bitcoin Time Series for Economic Research](https://arxiv.org/abs/2607.03124)
- [Bitcoin Mempool Linearization](https://arxiv.org/abs/2607.23787)
- [Strategies for quantum-enabled Bitcoin miners](https://arxiv.org/abs/2607.23952)
- [Optimizing Reward Mechanism in Pay-Per-Last-N-Shares Mining Pools under Block Withholding Attacks](https://www.sciencedirect.com/science/article/pii/S2096720926001016)
- [Unruly by Design: Fee Volatility and Strategic Attacks in Bitcoin Mining](https://www.nber.org/papers/w35443)
- [Searching for SHRINCS Parameters](https://blog.blockstream.com/searching-for-shrincs-parameters/)


## Network Data

- [Nodes OS Fingerprint analysis](https://bnoc.xyz/t/os-fingerprints-of-16-6k-bitcoin-nodes/152)

## Local/Legal

- [Russian State Duma passes law on regulation of digital currencies, digital rights  ](https://tass.com/economy/2163053)
- [ARD Doku: Der Krypto-Krieg - Wie Putin uns mit Bitcoins bekämpft](https://www.ardmediathek.de/tv-programm/6a33ad06b7131b8c5ffdf537)
- [Brivat Bitcoin Privacy event in Berlin](https://pretix.eu/fulmo/brivat/)

## Miscellaneous

- [Notice of ICANN .bitcoin TLD community application](https://x.com/wiz/status/2085029140453830725)
- [Bitcoin Antithesis](https://github.com/dergoegge/bitcoin-antithesis)
- [Introducing Bedrock: A Deterministic Hypervisor](https://brink.dev/blog/2026/06/25/bedrock-deterministic-hypervisor/)
- [Working on incremental snapshots](https://crypt-iq.github.io/2026/08/07/incrementalsnapshots.html)
- [Bitcoin Security Consortium](https://x.com/BTCconsortium/status/2080262540047585543)
- [BitMEX shuts down](https://www.bitmex.com/blog/bitmex-closure)
- [ElectrumX 2.0 release](https://electrumx-spesmilo.readthedocs.io/en/latest/changelog.html#version-2-0-0-03-july-2026)
- [First StratumV2 block mined](https://x.com/StratumV2/status/2070131857459429484)
- [Boltz disabled until further notice](https://x.com/Boltzhq/status/2084311537502630319)
- [Post-Mortem: What Happened Between Samourai Wallet and Me](https://nopara73.medium.com/samourai-wallet-post-mortem-900fa541fde5)
- [Bitdevs Map](https://www.bitdevsmap.org/)
- [Orange Juice](https://orangejuice.com/)
- [Polymarket accepts lightning via Spark](https://x.com/spark/status/2074539080197775421)

## Mailing Lists

### bitcoin-dev

- [Motion to remove Luke Dashjr from BIP Editors](https://groups.google.com/g/bitcoindev/c/knbv3MFwlvU)
- [BIP Proposal: Stale Tip Relay](https://groups.google.com/g/bitcoindev/c/AwOPNxF15mU)

### Delving Bitcoin

- [Libshrincs: A C implementation with a machine-checked security proof](https://delvingbitcoin.org/t/libshrincs-a-c-implementation-with-a-machine-checked-security-proof/2795/1)
- [Segwit commitment to post-quantum witness data?](https://delvingbitcoin.org/t/segwit-commitment-to-post-quantum-witness-data/2702)
- [PQC output type discussion](https://delvingbitcoin.org/t/pqc-output-type-discussion/2749)
- [Expiring HTLCs without free relay](https://delvingbitcoin.org/t/expiring-htlcs-without-free-relay/2663)

## Pull requests

### BIPs, BOLTs and Proposals

- [BIP458: Half-Aggregation of BIP 340 Signatures](https://github.com/bitcoin/bips/pull/2205)
- [BIP459: DahLIAS fully aggregated signatures for secp256k1](https://github.com/bitcoin/bips/pull/2210)
- [BIP460: CISA for Taproot Key Path Spends](https://github.com/bitcoin/bips/pull/2212)

### secp256k1

- [SECURITY.md: remove Jonas Nick from trusted keys #1884](https://github.com/bitcoin-core/secp256k1/pull/1884)
- [Make theStack a maintainer and a security contact #1887](https://github.com/bitcoin-core/secp256k1/pull/1887)
- [Add "silentpayments" module implementing BIP352 (take 4, limited to full-node scanning) #1765](https://github.com/bitcoin-core/secp256k1/pull/1765)
- [Silent Payments decrypt later](https://conduition.io/cryptography/hndl-silent-payments/)

### Bitcoin Core

- [p2p: Replace per-peer transaction rate-limiting with global rate limits #34628](https://github.com/bitcoin/bitcoin/pull/34628)
- [validation: fetch block input prevouts in parallel during ConnectBlock #35295](https://github.com/bitcoin/bitcoin/pull/35295)
- [wallet: Add exportwatchonlywallet RPC to export a watchonly version of a wallet #32489](https://github.com/bitcoin/bitcoin/pull/32489)
- [rpc: support a formal description of our JSON-RPC interface #34683](https://github.com/bitcoin/bitcoin/pull/34683)
- [p2p: Increase inbound capacity for block-relay only connections #28463](https://github.com/bitcoin/bitcoin/pull/28463)
- [wallet: store all witness variants of a transaction #35501](https://github.com/bitcoin/bitcoin/pull/35501)

### LND

- [bolt12: add InvoiceRequest codec and structural validators #10832](https://github.com/lightningnetwork/lnd/pull/10832)

### Core Lightning

- [Renepay too far #9324](https://github.com/ElementsProject/lightning/pull/9324)
- [wallet: add bwatch-backed output and transaction tables #9298](https://github.com/ElementsProject/lightning/pull/9298)

### BDK/rust-bitcoin & LDK/rust-lightning

- [p2p: Add support for BIP434 feature message #6364](https://github.com/rust-bitcoin/rust-bitcoin/pull/6364)
